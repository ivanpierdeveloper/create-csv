...
btnScaricaFileCsv.addEventListener('click', function() {
    ...
    if (question) {
        ...
        fnc.showMyAlert("AVVISO", "non hai scritto nome file", "var(--warning)", "var(--danger)", "var(--dark)", "var(--danger)");
    }
});
// func create file csv
...
btnZip.addEventListener('click', async function() {
    ...
    */
    ...
    frmData.append('create_zip', 'CRETE FILE ZIP');
    frmData.append('files', 'auto-1.gif');
    frmData.append('files', 'auto-2.gif');
    ...
    /*
    var json1 = await Promise.resolve({ "message": "msg-1" });
    console.warn(json1.message);
    */
    /*
        // begin funziona tutto benissimo
        await fetch(request)
            .then((response) => {
                if (response.ok) {
    ...
    */
    ...
    window.open("../create-file/docs/page/zip.php");
    /* console.group('begin');
